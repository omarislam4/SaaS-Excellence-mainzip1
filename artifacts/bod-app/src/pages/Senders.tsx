  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Senders</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Manage task sources and senders</p>
        </div>
        <motion.button
      ...
      {showCreate && (
        ...
        >
          <h3 className="text-sm font-semibold text-foreground mb-4">Add Sender</h3>
          <form onSubmit={handleCreate} className="space-y-4">
            <div className="grid sm:grid-cols-2 gap-4">
              {[
                { key: "name", label: "Name", placeholder: "John Smith", required: true },
                { key: "company", label: "Company", placeholder: "Company Inc." },
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button type="submit" className="px-4 py-2 text-sm font-semibold bg-primary text-primary-foreground rounded-xl hover:bg-primary/90 transition-colors" data-testid="submit-sender">Add Sender</button>
            </div>
          </form>
        </motion.div>
      )}
      ...
      {loading ? (
        ...
        <EmptyState icon={Send} title="No senders yet" description="Add senders to track where tasks originate." action={{ label: "Add Sender", onClick: () => setShowCreate(true) }} />
      ) : (
          ...
          {senders.map((sender, i) => (
            ...
            >
              ...
              <div className="flex items-center gap-1">
                {editing === sender.id ? (
                  <>
                    <button onClick={() => handleUpdate(sender.id)} className="p-1.5 rounded-lg text-emerald-500 hover:bg-emerald-500/10 transition-colors" data-testid={`save-sender-${sender.id}`}><Check className="w-4 h-4" /></button>
                  ...
                  <>
                    <button onClick={() => { setEditing(sender.id); setEditForm({}); }} className="p-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted transition-colors" data-testid={`edit-sender-${sender.id}`}><Edit2 className="w-3.5 h-3.5" /></button>
                    <button onClick={() => handleDelete(sender.id)} className="p-1.5 rounded-lg text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors" data-testid={`delete-sender-${sender.id}`}><Trash2 className="w-3.5 h-3.5" /></button>
                  </>
                )}
              </div>
            </motion.div>
